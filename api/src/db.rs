...
pub async fn get_user_settings(
    ...
    let row: Option<(Option<String>, String)> =
        sqlx::query_as("SELECT account_badge, theme FROM user_settings WHERE user_id = $1")
            .bind(user_id)
            .fetch_optional(pool)
            .await?;
    let (account_badge, theme) = match row {
        Some((b, t)) => (b, t),
        // No row yet: account_badge defaults to None, theme defaults to
        // 'classic' here too, mirroring the column's DB-level default.
...
/// Upsert the caller's account-scoped settings. `account_badge` and
/// `theme` are exposed today; new fields just need adding here and to
/// UserSettings. Pass None to clear account_badge.
pub async fn upsert_user_settings(
    ...
    sqlx::query(
        "INSERT INTO user_settings (user_id, account_badge, theme, updated_at)
         VALUES ($1, $2, $3, now())
