...
async fn main() -> Result<()> {
    ...
    s3_client
        ...
        .send()
        .await
        .with_context(|| format!("upload to bucket {bucket_name} with key {key}"))?;

    // List objects in the bucket
    let list_objects_res = s3_client
        .list_objects_v2()
        .bucket(bucket_name)
        .send()
        .await
        .with_context(|| format!("list objects in bucket {bucket_name}"))?;
    ...
    let get_object_res = s3_client
        ...
        .send()
        .await
        .with_context(|| format!("download {bucket_name} with key {key}"))?;
    let downloaded_data = get_object_res
        .body
        .collect()
        .await
